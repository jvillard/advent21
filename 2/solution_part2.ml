...
type position = {h: int; v: int; aim: int}

let initial_pos = {h= 0; v= 0; aim= 0}

let pp_position fmt {h; v; aim} = Format.fprintf fmt "{h=%d; v=%d; aim= %d}" h v aim

let hash {h; v} = h * v

let move_one ({h= h0; v= v0; aim= aim0} as pos0) move =
  match move with
  | Forward x ->
      {pos0 with h= h0 + x; v= v0 + (aim0 * x)}
  | Down x ->
      {pos0 with aim= aim0 + x}
  | Up x ->
      {pos0 with aim= aim0 - x}
